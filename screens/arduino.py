...
def abrir_janela_arduino(root):
    ...
    label_dispositivo = ttk.Label(
        container, text="Selecione o dispositivo ESP32:")
    label_dispositivo.pack(pady=(10, 5))

    combobox_dispositivo = ttk.Combobox(
        container, values=dispositivos, state="readonly")
    if dispositivos:
        combobox_dispositivo.pack(pady=(0, 20))
        combobox_dispositivo.current(0)
    else:
        label_no_device = ttk.Label(
            container, text="Não existe nenhum dispositivo ESP32 cadastrado", font=("Helvetica", 14))
        label_no_device.pack(pady=(10, 5))

    # Configuração da porta serial inicial
    serial_connection_arduino, _ = open_serial_connection('COM4')
    if not serial_connection_arduino:
        messagebox.showerror(
            "Erro Serial", "Não foi possível abrir a conexão serial na porta COM4.")
    ...
    # Atualiza os campos baseado na conexão serial
    def update_fields():
        global serial_connection_arduino
        existing_data = get_existing_data(serial_connection=serial_connection_arduino)
        can_generate_id = True
        if existing_data:
            nome_dispositivo.set(existing_data['nome'])
            id_var.set(existing_data['id'])
            can_generate_id = False
        # entry_id.configure(state='normal' if can_generate_id else 'disabled')
    ...
    def on_port_selected(event):
        ...
        if success:
            ...
            update_fields()
            messagebox.showinfo(
                "Conexão Serial", f"Conexão estabelecida com sucesso na porta {selected_port}.")
        else:
            messagebox.showerror(
                "Conexão Serial", f"Falha ao estabelecer conexão na porta {selected_port}.")
    ...
    entry_id = ttk.Entry(container, width=40, textvariable=id_var)
    ...
    voltar_button = tk.Button(janela_arduino, text="← Voltar",
                              command=lambda: fecha_janela.close_and_return(janela_arduino))
    voltar_button.place(x=10, y=10)
    janela_arduino.protocol(
        "WM_DELETE_WINDOW", lambda: fecha_janela.close_and_return(janela_arduino))
